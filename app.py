...
# Sidebar - Pilihan Menu Utama
st.sidebar.header("📌 Pilih Jenis Data")
menu_option = st.sidebar.radio(
    "Pilih kategori data yang ingin ditarik:",
    ["Produk Varian", "Produk Cangkang (Template)"]
)

# Sidebar - Konfigurasi Request
...
batch_size = st.sidebar.number_input(
    ...
    value=100, 
    step=10,
    help="Ketik jumlah item per halaman secara manual."
)
...
search_keyword = st.sidebar.text_input(
    "Kata Kunci Pencarian (contoh: 'cath' atau 'syringe')", 
    value="cath"
)
...
# Reset state jika pengguna berpindah menu
if "current_menu" not in st.session_state or st.session_state["current_menu"] != menu_option:
    st.session_state["current_menu"] = menu_option
    st.session_state["all_fetched_data"] = None

# ==========================================
# MENU 1: PRODUK VARIAN
# ==========================================
if menu_option == "Produk Varian":
    st.markdown("### 🔹 Penarikan Data Produk Varian")
    ...
    if start_download:
        ...
        while True:
            status_text.info(f"⏳ Sedang mengambil data Varian Halaman **{page}** ({batch_size} item per request)...")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    for item in items:
                        ...
                        variant_name = item.get("product_variant_name") or item.get("name") or item.get("productVariantName") or ""
                        nie_no = item.get("nie") or item.get("nie_number") or ""
                        
                        registrar_obj = item.get("registrar") or item.get("registrar_name") or ""
                        registrar_name = registrar_obj.get("name") if isinstance(registrar_obj, dict) else str(registrar_obj)
                            
                        manufacturer_obj = item.get("manufacturer") or item.get("manufacturer_name") or ""
                        manufacturer_name = manufacturer_obj.get("name") if isinstance(manufacturer_obj, dict) else str(manufacturer_obj)
...
# ==========================================
# MENU 2: PRODUK CANGKANG (TEMPLATE)
# ==========================================
else:
    st.markdown("### 📦 Penarikan Data Produk Cangkang (Template)")
    
    selected_columns = ["Kode KFA", "Nama produk cangkang"]
    st.info("ℹ️ Menu ini khusus menarik 2 kolom utama: **Kode KFA** dan **Nama produk cangkang**.")

    st.divider()
    start_download = st.button("🚀 Mulai Penarikan Data Produk Cangkang", type="primary")
    ...
    if start_download:
        ...
        while True:
            status_text.info(f"⏳ Sedang mengambil data Cangkang Halaman **{page}** ({batch_size} item per request)...")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if not items:
                        status_text.success(f"✅ Penarikan data selesai! Total **{len(all_rows)}** data cangkang berhasil diambil.")
                        break
                        
                    for item in items:
                        kfa_code = item.get("kfaCode") or item.get("kfa_code") or ""
                        template_name = item.get("productTemplateName") or item.get("name") or ""

                        row = {
                            "Kode KFA": kfa_code,
                            "Nama produk cangkang": template_name
                        }
                        all_rows.append(row)
                    
                    status_text.info(f"🔄 Halaman {page}: Berhasil mengambil **{len(items)}** data cangkang (Total Sementara: **{len(all_rows)}** data)")
                    
                    df_current = pd.DataFrame(all_rows)
                    table_placeholder.dataframe(df_current.tail(10), use_container_width=True)
                    
                    if len(items) < batch_size:
                        status_text.success(f"✅ Mencapai akhir halaman. Total **{len(all_rows)}** data cangkang berhasil diambil.")
                        break
...
# ==========================================
# SEKSI DOWNLOAD FILE TXT (DAPAT DIGUNAKAN KEDUA MENU)
# ==========================================
if st.session_state["all_fetched_data"]:
    ...
    file_prefix = "kfa_alkes_produk_varian" if menu_option == "Produk Varian" else "kfa_alkes_produk_cangkang"
